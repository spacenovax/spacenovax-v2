...
function PersonalMessages({ user, setUser, language, onRead }) {
  const ko = language === 'ko';
  const [messages, setMessages] = useState([]);
  const [members, setMembers] = useState([]);
  const [recipientId, setRecipientId] = useState('');
  const [query, setQuery] = useState('');
  const [nickname, setNickname] = useState(user.communityNickname || '');
  const [nicknameState, setNicknameState] = useState(user.communityNickname ? 'saved' : 'idle');
  const [draft, setDraft] = useState({ body:'', imageData:'', imageName:'', replyTo:'' });
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { const timer=setTimeout(()=>api('/api/messages/members',{method:'POST',body:{query}}).then((data)=>setMembers(data.members||[])).catch((error)=>setNotice(error.message)),250); return()=>clearTimeout(timer); },[query]);
  useEffect(() => { if(!nickname||nickname===user.communityNickname){setNicknameState(user.communityNickname?'saved':'idle');return} const timer=setTimeout(()=>api('/api/profile/nickname/check',{method:'POST',body:{nickname}}).then((data)=>setNicknameState(data.available?'available':'unavailable')).catch(()=>setNicknameState('unavailable')),350); return()=>clearTimeout(timer); },[nickname,user.communityNickname]);
  async function markAllRead() {
    try { await api('/api/messages/read', { method: 'POST', body: { messageId: 'all' } }); onRead?.(); await load(); }
    catch (error) { setNotice(error.message); }
  }
  async function attachPhoto(file) { if(!file)return; if(!/^image\/(png|jpeg|webp)$/.test(file.type))return setNotice(ko?'JPG, PNG 또는 WebP 사진만 가능합니다.':'Use JPG, PNG, or WebP.'); try{const source=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(file)});const image=await new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=reject;img.src=source});const scale=Math.min(1,1400/Math.max(image.width,image.height));const canvas=document.createElement('canvas');canvas.width=Math.round(image.width*scale);canvas.height=Math.round(image.height*scale);canvas.getContext('2d').drawImage(image,0,0,canvas.width,canvas.height);const imageData=canvas.toDataURL('image/jpeg',.8);if(imageData.length>1_350_000)return setNotice(ko?'사진 용량이 너무 큽니다.':'Photo is too large.');setDraft((current)=>({...current,imageData,imageName:file.name}))}catch(error){setNotice(error.message)} }
  async function sendMessage(){if(!recipientId)return setNotice(ko?'받는 회원을 선택하세요.':'Choose a recipient.');try{const data=await api('/api/messages/send',{method:'POST',body:{recipientId,...draft}});setDraft({body:'',imageData:'',imageName:'',replyTo:''});setNotice(data.message);await load()}catch(error){setNotice(error.message)}}
  async function toggleBlock(item){const targetId=item.direction==='sent'?item.userId:item.senderId;try{await api('/api/messages/block',{method:'POST',body:{userId:targetId,action:item.blocked?'unblock':'block'}});setNotice(item.blocked?(ko?'차단을 해제했습니다.':'Member unblocked.'):(ko?'회원 쪽지를 차단했습니다.':'Member blocked.'));await load()}catch(error){setNotice(error.message)}}
  async function reportMessage(item){if(!window.confirm(ko?'사기·운영자 사칭 또는 피싱 쪽지로 신고하고 발신자를 차단합니까?':'Report this message for fraud, impersonation, or phishing and block the sender?'))return;try{const data=await api('/api/messages/report',{method:'POST',body:{messageId:item.id,reason:'fraud_impersonation_or_phishing'}});setNotice(data.message);await load()}catch(error){setNotice(error.message)}}
  async function saveNickname(){if(nicknameState!=='available'&&nickname!==user.communityNickname)return;try{const data=await api('/api/profile/nickname',{method:'POST',body:{nickname}});if(data.user)setUser((current)=>({...current,...data.user}));setNicknameState('saved');setNotice(ko?'닉네임이 저장되었습니다.':'Nickname saved.')}catch(error){setNicknameState('unavailable');setNotice(error.message)}}
  const nicknameCard = user.missionPassportComplete ? <div className="message-nickname-card"><div><small>UNIQUE COMMUNITY NICKNAME</small><b>{ko?'회원에게 표시할 닉네임':'Nickname shown to members'}</b><p>{ko?'2~20자 · 중복 닉네임은 사용할 수 없습니다.':'2–20 characters · Duplicate nicknames are blocked.'}</p></div><input value={nickname} maxLength={20} onChange={(e)=>setNickname(e.target.value)} placeholder={ko?'닉네임 입력':'Enter nickname'}/><span className={nicknameState}>{nicknameState==='available'?(ko?'사용 가능':'AVAILABLE'):nicknameState==='unavailable'?(ko?'사용 불가·중복':'UNAVAILABLE'):nicknameState==='saved'?(ko?'등록 완료':'SAVED'):(ko?'확인 대기':'CHECKING')}</span><button disabled={nicknameState!=='available'&&nickname!==user.communityNickname} onClick={saveNickname}>{ko?'닉네임 저장':'SAVE NICKNAME'}</button></div> : null;
  return <main className="v15-page"><section className="command-card ops-module personal-message-module">
    <div className="section-heading"><div><small>CAPTAIN PRIVATE CHANNEL</small><h2>{ko ? '개인 쪽지' : 'Personal Messages'}</h2></div><button className="node-guide-link" onClick={markAllRead}>{ko ? '전체 읽음' : 'MARK ALL READ'}</button></div><div className="message-security-warning"><Icon name="shield" size={20}/><div><b>{ko?'사기·사칭 계정 영구정지':'FRAUD & IMPERSONATION · PERMANENT SUSPENSION'}</b><p>{ko?'사기성 쪽지, 금전·지갑 정보 요구, 운영자 사칭 또는 피싱 링크가 확인되면 사전 통보 없이 계정이 영구 정지됩니다. SpaceNovaX 운영자는 개인 키, 시드 문구, 비밀번호 또는 개인 송금을 절대 요구하지 않습니다.':'Accounts confirmed to send fraudulent messages, request funds or wallet secrets, impersonate administrators, or distribute phishing links will be permanently suspended without prior notice. SpaceNovaX never requests private keys, seed phrases, passwords, or personal payments.'}</p></div></div>{nicknameCard}
    <div className={`member-message-access ${user.missionPassportComplete?'unlocked':''}`}><Icon name={user.missionPassportComplete?'community':'mission'} size={20}/><div><b>{user.missionPassportComplete?(ko?'회원 쪽지 사용 가능':'Member messaging unlocked'):(ko?'5대 미션 완료 후 회원 쪽지 가능':'Complete five missions to message members')}</b><p>{ko?'5대 공식 미션 완료 회원끼리 글·사진 쪽지를 주고받을 수 있습니다.':'Members who complete all five missions can exchange private text and photo messages.'}</p></div></div>
    {user.missionPassportComplete&&user.communityNickname&&<div className="member-message-composer"><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder={ko?'회원 이름 검색':'Search member name'}/><select value={recipientId} onChange={(e)=>setRecipientId(e.target.value)}><option value="">{ko?'받는 회원 선택':'Choose recipient'}</option>{members.map((member)=><option value={member.id} key={member.id}>{member.firstName}{member.username?` · @${member.username}`:''}</option>)}</select>{draft.replyTo&&<p className="message-reply-preview">↩ {ko?'답장 작성 중':'Replying to message'} <button onClick={()=>setDraft({...draft,replyTo:''})}>×</button></p>}<textarea maxLength={1500} value={draft.body} onChange={(e)=>setDraft({...draft,body:e.target.value})} placeholder={ko?'비공개 쪽지를 입력하세요.':'Write a private message.'}/><div><label><Icon name="plus" size={14}/>{draft.imageName||(ko?'사진 첨부':'ATTACH PHOTO')}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e)=>attachPhoto(e.target.files?.[0])}/></label><button onClick={sendMessage}>{ko?'쪽지 보내기':'SEND MESSAGE'}<Icon name="arrow" size={14}/></button></div>{draft.imageData&&<img className="message-photo-preview" src={draft.imageData} alt=""/>}</div>}
    <div className="personal-message-list">{messages.length ? messages.map((item) => <article className={`${item.readAt||item.direction==='sent'?'':'unread'} ${item.direction==='sent'?'sent':''}`} key={item.id}>{item.type==='member_message'?(item.senderAvatarUrl?<img className="message-member-avatar" src={item.senderAvatarUrl} alt=""/>:<span className="message-member-avatar">{(item.senderName||'C').slice(0,1)}</span>):<i className={item.type === 'node_online' ? 'online' : item.type === 'node_offline' ? 'offline' : ''}/>}<div><small>{item.type==='member_message'?(item.direction==='sent'?(ko?`보낸 쪽지 · ${item.recipientName}`:`SENT · ${item.recipientName}`):(ko?`받은 쪽지 · ${item.senderName}`:`FROM · ${item.senderName}`)):item.type.replaceAll('_',' ').toUpperCase()}</small><b>{item.title}</b>{item.replyPreview&&<blockquote>↩ {item.replyPreview}</blockquote>}<p>{item.body}</p>{item.imageUrl&&<img className="message-photo" src={item.imageUrl} alt="Message attachment" loading="lazy"/>}<time>{new Date(item.createdAt).toLocaleString()}</time>{item.type==='member_message'&&<nav>{item.direction!=='sent'&&<button onClick={()=>{setRecipientId(item.senderId);setDraft((current)=>({...current,replyTo:item.id}));window.scrollTo({top:0,behavior:'smooth'})}}>{ko?'답장':'REPLY'}</button>}<button onClick={()=>toggleBlock(item)}>{item.blocked?(ko?'차단 해제':'UNBLOCK'):(ko?'차단':'BLOCK')}</button>{item.direction!=='sent'&&<button className="danger" onClick={()=>reportMessage(item)}>{ko?'사기·사칭 신고':'REPORT FRAUD'}</button>}</nav>}</div></article>) : <p className="module-notice">{ko ? '아직 받은 쪽지가 없습니다.' : 'No personal messages yet.'}</p>}</div>
...
function NodeSetup({ language, setTab }) {
  ...
  const download = 'https://github.com/spacenovax/spacenovax-v2/releases/download/node-v1.1.0/Genesis_Community_Node_V1.1_Windows_x64.zip';
...
export default function V15App() {
  ...
  else if (tab === 'messages') page = <PersonalMessages user={user} setUser={setUser} language={language} onRead={sync}/>;
