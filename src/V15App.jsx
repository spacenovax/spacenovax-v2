...
function PersonalMessages({ user, setUser, language, onRead }) {
  ...
  return <main className="v15-page"><section className="command-card ops-module personal-message-module">
    <div className="section-heading"><div><small>CAPTAIN PRIVATE CHANNEL</small><h2>{ko ? '개인 쪽지' : 'Personal Messages'}</h2></div><button className="node-guide-link" onClick={markAllRead}>{ko ? '전체 읽음' : 'MARK ALL READ'}</button></div>{nicknameCard}
