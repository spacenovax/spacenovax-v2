...
function Header({ user, language, setLanguage, t, onPreview, onMessages, onAnnouncements }) {
  ...
  return <header className="v15-header">
    ...
    <div className="header-actions">
      ...
      <button className="announcement-bell" onClick={onAnnouncements} aria-label="Global announcements"><Icon name="mission" size={17}/>{Number(user.unreadAnnouncementCount || 0) > 0 && <b>{Math.min(99, Number(user.unreadAnnouncementCount || 0))}</b>}</button>
      <button className="message-bell" onClick={onMessages} aria-label="Personal messages"><Icon name="community" size={17}/>{Number(user.unreadMessageCount || 0) > 0 && <b>{Math.min(99, Number(user.unreadMessageCount || 0))}</b>}</button>
